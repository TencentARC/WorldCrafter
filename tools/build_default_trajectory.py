...
ROOT = Path(__file__).resolve().parents[1] / "test" / "poses"
CHUNK_FRAMES = 33
FPS = 16
EVENTS = (
    *("f1",) * 2,
...
for chunk_index, event in enumerate(EVENTS):
    ...
    if kind in {"f", "b", "l", "r"}:
        ...
        for frame_index, fraction in enumerate(chunk_alpha):
            poses[frame_index, :3, :3] = rotation_y(signed * fraction) @ start[:3, :3]
...
if not np.allclose(global_pose[-1], np.eye(4), atol=1.0e-12, rtol=0.0):
    raise AssertionError(f"final trajectory frame is not identity:\n{global_pose[-1]}")
for chunk_index in range(1, len(EVENTS)):
    ...
    rotation = math.degrees(math.acos(np.clip((np.trace(relative) - 1.0) / 2.0, -1.0, 1.0)))
...
(ROOT / "trajectory.json").write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n")
print(json.dumps({key: manifest[key] for key in ("num_chunks", "num_frames", "motion_sequence", "logical_endpoint_identity_max_error", "sha256")}, indent=2))
