...
def _get_qkv_projections(attn: "WorldCoreAttention", hidden_states: torch.Tensor, encoder_hidden_states: torch.Tensor):
...
class WorldCoreOutputNorm(nn.Module):
...
class WorldCoreAttnProcessor:
    ...
    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError(
                "WorldCoreAttnProcessor requires PyTorch 2.0. To use it, please upgrade PyTorch to version 2.0 or higher."
            )

    def __call__(
        self,
        attn: "WorldCoreAttention",
        hidden_states: torch.Tensor,
...
class WorldCoreAttention(torch.nn.Module, AttentionModuleMixin):
    _default_processor_cls = WorldCoreAttnProcessor
    _available_processors = [WorldCoreAttnProcessor]
...
class WorldCoreTimeTextEmbedding(nn.Module):
    def __init__(
        self,
        dim: int,
        time_freq_dim: int,
...
class WorldCoreRotaryPosEmbed(nn.Module):
...
class WorldCoreTransformerBlock(nn.Module):
    def __init__(
        self,
        dim: int,
        ffn_dim: int,
        ...
        self.attn1 = WorldCoreAttention(
            dim=dim,
            heads=num_heads,
            dim_head=dim // num_heads,
            eps=eps,
            cross_attention_dim_head=None,
            processor=WorldCoreAttnProcessor(),
            is_amplify_history=is_amplify_history,
            history_scale_mode=history_scale_mode,
        )

        # 2. Cross-attention
        self.attn2 = WorldCoreAttention(
            dim=dim,
            heads=num_heads,
            dim_head=dim // num_heads,
            eps=eps,
...
class WorldCoreTransformer3DModel(
    ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin, AttentionMixin
):
    r"""
    A Transformer model for video-like data used in the WorldCore model.
    ...
    _no_split_modules = ["WorldCoreTransformerBlock", "WorldCoreOutputNorm"]
    ...
    _repeated_blocks = ["WorldCoreTransformerBlock"]
    _cp_plan = {
    ...
    def __init__(
        ...
        self.rope = WorldCoreRotaryPosEmbed(rope_dim=rope_dim, theta=rope_theta)
        ...
        self.condition_embedder = WorldCoreTimeTextEmbedding(
            dim=inner_dim,
        ...
        self.norm_out = WorldCoreOutputNorm(inner_dim, eps, elementwise_affine=False)
