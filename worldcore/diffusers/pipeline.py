...
from .pipeline_output import WorldCorePipelineOutput
from .scheduler import WorldCoreScheduler
from .transformer import WorldCoreTransformer3DModel
...
def _render_repencoder_memory_latents(
    *,
    ...
    if generated_latents.ndim != 5 or generated_latents.shape[2] == 0:
        raise RuntimeError(
            "RepEncoder requires a non-empty [B,C,T,H,W] bank of already generated WorldCore latents"
        )
    ...
    if memory_latents.device != generated_latents.device:
        raise ValueError(
            "RepEncoder memory must stay on the WorldCore latent device; "
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from diffusers import AutoencoderKLWan, WorldCorePipeline

        >>> # Available models: BestWishYsh/WorldCore-Base, BestWishYsh/WorldCore-Mid, BestWishYsh/WorldCore-Distilled
        >>> model_id = "BestWishYsh/WorldCore-Base"
        >>> vae = AutoencoderKLWan.from_pretrained(model_id, subfolder="vae", torch_dtype=torch.float32)
        >>> pipe = WorldCorePipeline.from_pretrained(model_id, vae=vae, torch_dtype=torch.bfloat16)
        >>> pipe.to("cuda")
...
class WorldCorePipeline(DiffusionPipeline, _BaseLoraLoaderMixin):
    r"""
    Pipeline for text-to-video / image-to-video / video-to-video generation using WorldCore.
    ...
    Args:
        ...
        transformer ([`WorldCoreTransformer3DModel`]):
            Conditional Transformer to denoise the input latents.
        scheduler ([`WorldCoreScheduler`]):
    ...
    def __call__(
        ...
        Args:
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether or not to return a [`WorldCorePipelineOutput`] instead of a plain tuple.
        ...
        Returns:
            [`~WorldCorePipelineOutput`] or `tuple`:
                If `return_dict` is `True`, [`WorldCorePipelineOutput`] is returned, otherwise a `tuple` is returned where
        ...
        if image is not None and video is not None:
            raise ValueError("image and video cannot be provided simultaneously")
        if camera_trajectory is not None and is_enable_stage2:
            raise NotImplementedError("UCPE camera inference is wired to WorldCore stage1 only")
        if memory_size != 4:
        ...
        if resume_state is not None:
            if resume_state.get("format") != "worldcore_chunk_state_v1":
                raise ValueError("unsupported WorldCore resume-state format")
            ...
            if not isinstance(generator, torch.Generator):
                raise TypeError("resumable WorldCore inference requires one torch.Generator")
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                ...
                if callback_on_chunk_state is not None:
                    ...
                    callback_on_chunk_state(
                        k,
                        {
                            "format": "worldcore_chunk_state_v1",
        ...
        return WorldCorePipelineOutput(frames=video)
