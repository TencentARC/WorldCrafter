...
def _as_global_metric_c2w(
    ...
    if pose.shape[0] != int(batch_size):
        raise ValueError(
            f"{pose_key} batch {pose.shape[0]} does not match WorldCore latent batch {batch_size}"
        )
    if int(batch_size) != 1:
    ...
    def _validate_latents(
        ...
        *,
        ...
        if tuple(generated_latents.shape[1:]) != expected_tail:
            raise ValueError(
                "generated_latents do not match complete causal WorldCore history: "
        ...
        if torch.device(runtime_device) != generated_latents.device:
            raise ValueError(
                "RepEncoder runtime and WorldCore latents must share one device; "
                f"runtime={runtime_device}, worldcore={generated_latents.device}"
            )
    ...
    def render_memory(
        ...
        *,
        ...
        if not torch.equal(source_latents[:, 0], recent_latents[:, :, 0]):
            raise RuntimeError("assembled RepEncoder source[0] differs from WorldCore recent[-1]")
        ...
        if memory4.device != generated_latents.device:
            raise ValueError("RepEncoder memory4 must remain on the WorldCore latent device")
